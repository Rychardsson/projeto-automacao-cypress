  ...
  try {
    ...
    fornecedores.forEach((fornecedor) => {
      ...
      row.innerHTML = `
                ...
                <td>
                    <span class="badge ${
                      fornecedor.ativo ? "bg-success" : "bg-danger"
                    }">
                        ${fornecedor.ativo ? "Ativo" : "Inativo"}
                    </span>
                </td>
                <td>
                    <button class="btn btn-sm btn-outline-primary me-1" onclick="editarFornecedor(${
                      fornecedor.id
                    })" data-cy="btn-editar">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-danger" onclick="excluirFornecedor(${
                      fornecedor.id
                    })" data-cy="btn-excluir">
...
function novoFornecedor() {
  ...
  document.getElementById("modalFornecedorLabel").textContent =
    "Novo Fornecedor";
...
async function salvarFornecedor() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const mensagem = editandoFornecedor
